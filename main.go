...
func main() {
	...
	r.GET("/roast", func(c *gin.Context) {
		c.HTML(http.StatusOK, "roast.html", nil)
	})

	// Route to handle the form submission and make the API request
	r.POST("/roast", func(c *gin.Context) {
		// Get the prompt from the form
		name := c.PostForm("name")
		proud := c.PostForm("proud")
		embarassing := c.PostForm("embarassing")

		// Call the function to generate the article using the API
		roast, err := generateRoast(name, proud, embarassing)
		if err != nil {
			log.Printf("Error generating article: %v", err)
			c.String(http.StatusInternalServerError, "Error generating article")
			return
...
// Function to generate the article by calling the API
func generateRoast(name string, proud string, embarassing string) (string, error) {
	url := "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=AIzaSyAB22gdjZYFhFdRO3qnSODsXwA-Sz0Qpgw"
	...
	requestBody := map[string]interface{}{
		"contents": []map[string]interface{}{
			{
				"parts": []map[string]string{
					{"text": "Roast me, like the presidential roast. Avoid just corny one-liners. I want a refined, professional roast. My name is " + name + ". I am proud of: " + proud + ". I am embarassed about: " + embarassing},
				},
			},
		},
	}
